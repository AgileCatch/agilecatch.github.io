...
1. Ruby를 업데이트하기 위해 RVM (Ruby Version Manager)을 설치한다.

```bash
\curl -sSL https://get.rvm.io | bash -s stable
```

2. RVM을 활성화합니다. 터미널에서 다음 명령을 실행한다.

```bash
source ~/.rvm/scripts/rvm
```

3. Ruby 2.7.0을 설치합니다. 터미널에서 다음 명령을 실행한다.

```bash
rvm install 2.7.0
```

4. 설치된 Ruby 2.7.0을 사용하도록 설정합니다. 터미널에서 다음 명령을 실행한다.

```bash
rvm use 2.7.0 --default
```

5. Ruby 버전이 업데이트되었으므로 CocoaPods를 설치할 수 있다. 터미널에서 다음 명령을 실행한다.
...
1. 터미널에서 다음 명령을 실행하여 Ruby를 업데이트합니다.
...
2. Ruby 업데이트 후에는 다음 명령으로 `gem`을 업데이트합니다.

```bash
gem update --system
```

3. 이제 다시 CocoaPods을 설치해보자.
