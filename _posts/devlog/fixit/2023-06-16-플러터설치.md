...
Ruby를 업데이트하기 위해 RVM (Ruby Version Manager)을 설치합니다. 
```bash
\curl -sSL https://get.rvm.io | bash -s stable
```

RVM을 활성화합니다. 터미널에서 다음 명령을 실행합니다.
```bash
source ~/.rvm/scripts/rvm
```

Ruby 2.7.0을 설치합니다. 터미널에서 다음 명령을 실행합니다.

```bash
rvm install 2.7.0
```

설치된 Ruby 2.7.0을 사용하도록 설정합니다. 터미널에서 다음 명령을 실행합니다.

```bash
rvm use 2.7.0 --default
```

Ruby 버전이 업데이트되었으므로 CocoaPods를 설치할 수 있습니다. 터미널에서 다음 명령을 실행합니다.
