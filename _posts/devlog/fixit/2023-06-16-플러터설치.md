...
* RVM을 활성화합니다. 터미널에서 다음 명령을 실행한다.

```bash
source ~/.rvm/scripts/rvm
```

* Ruby 2.7.0을 설치합니다. 터미널에서 다음 명령을 실행한다.

```bash
rvm install 2.7.0
```

* 설치된 Ruby 2.7.0을 사용하도록 설정합니다. 터미널에서 다음 명령을 실행한다.
...
*  터미널에서 다음 명령을 실행하여 Ruby를 업데이트합니다.
...
* Ruby 업데이트 후에는 다음 명령으로 `gem`을 업데이트합니다.
